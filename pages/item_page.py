...
class ItemPage(Page):
    ...
    def render(self, draw):
        ...
        # hint only when active (иначе пусто/спокойно)
        if self.active:
            draw.text((x0 + 10, y1 - 18), "tap a socket to toggle",
                      fill=DIV_LINE, font=font_sm)

    def _draw_socket_row(self, draw, x0, y0, x1, y1, s, selected=False, inactive=False):
        mid_y = (y0 + y1) // 2

        # colors based on active/inactive
        if inactive:
            name_color = DIV_LINE
            sub_color = DIV_LINE
        else:
        ...
        draw.text((x0 + 6, y0 + 36), sub, fill=sub_color, font=font_sm)
        ...
        ty1 = ty0 + toggle_h
        self._draw_toggle(draw, tx0, ty0, tx1, ty1, s["state"], inactive=inactive)

    def _draw_toggle(self, draw, x0, y0, x1, y1, is_on, inactive=False):
        w = x1 - x0
        h = y1 - y0
        r = h // 2
        ...
        outline_col = dim if inactive else accent
        fill_col = bg

        # Track outline
        draw.rounded_rectangle([x0, y0, x1, y1], radius=r,
                               outline=outline_col, fill=fill_col, width=2)

        # ON fill (RIGHT segment only) — only if active
        if is_on and not inactive:
            pad = 2
        ...
        if inactive:
            draw.text((on_x, ty), on_txt, fill=dim, font=font_sm)
        else:
    ...
    def handle_touch(self, x, y, state):
        # если страница не активна — тач игнорим (по твоей логике "неактивные")
        if not self.active:
            return
